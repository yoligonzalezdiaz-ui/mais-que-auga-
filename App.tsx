...
const App: React.FC = () => {
  ...
  const getFeatureIcon = (feature: string) => {
    ...
    if (f.includes('piscina') || f.includes('spa') || f.includes('jacuzzi')) return <Waves className="w-5 h-5" />;
    if (f.includes('fitness') || f.includes('peso libre')) return <Dumbbell className="w-5 h-5" />;
    if (f.includes('crossfit') || f.includes('funcional')) return <Zap className="w-5 h-5" />;
    if (f.includes('parking')) return <Car className="w-5 h-5" />;
    if (f.includes('vestuarios') || f.includes('baño')) return <Bath className="w-5 h-5" />;
    if (f.includes('cafetería') || f.includes('nutrición')) return <Coffee className="w-5 h-5" />;
    if (f.includes('ludoteca') || f.includes('infantil')) return <Accessibility className="w-5 h-5" />;
    if (f.includes('sauna') || f.includes('turco')) return <Wind className="w-5 h-5" />;
      ...
      <header className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-4 lg:gap-8">
            ...
            <button onClick={() => setView('schedule')} className={`text-sm font-black transition-colors uppercase tracking-tight ${view === 'schedule' ? 'text-[#8dc63f]' : 'text-gray-400 hover:text-gray-900'}`}>Clases</button>
           ...
           {selectedCenter ? (
              <div className="animate-in fade-in duration-500">
                 ...
                 <div className="grid lg:grid-cols-12 gap-16">
                    <div className="lg:col-span-7 space-y-16">
                       <div>
                          <span className="text-[#8dc63f] font-black text-xs uppercase tracking-[0.4em] mb-4 block">Tour Virtual</span>
                       ...
                       <div>
                          <h3 className="text-4xl font-black tracking-tighter uppercase mb-10">Instalaciones Elite</h3>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                             {selectedCenter.features.map(f => (
                               <div key={f} className="p-8 bg-gray-50 rounded-[35px] border-2 border-gray-100 flex items-center gap-6 group hover:border-[#8dc63f] hover:bg-white hover:shadow-2xl transition-all">
                                  <div className="w-12 h-12 bg-[#8dc63f] rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform">
                                     {getFeatureIcon(f)}
                                  </div>
                                  <p className="font-black text-sm text-gray-800 uppercase tracking-tight">{f}</p>
                               </div>
                             ))}
                          </div>
                       </div>
                       ...
                       <div className="grid grid-cols-2 gap-6">
                          {selectedCenter.gallery.map((img, i) => (
                             <img key={i} src={img} className="rounded-[40px] h-64 w-full object-cover shadow-xl" alt="Gym Gallery" />
                          ))}
                       </div>
                    </div>
              ...
              <div className="space-y-20">
                 <div className="text-center max-w-2xl mx-auto">
                    ...
                    <p className="text-gray-500 text-lg font-medium">Elige tu sede de entrenamiento en Vigo. Cada centro cuenta con instalaciones únicas especializadas.</p>
                 </div>
                 <div className="grid md:grid-cols-3 gap-10">
                    {CENTERS.map(center => (
                       <div key={center.id} onClick={() => setSelectedCenter(center)} className="group bg-white border-4 border-gray-100 rounded-[60px] overflow-hidden hover:shadow-2xl transition-all cursor-pointer">
                          ...
                          <div className="p-10">
                             <h3 className="text-3xl font-black text-gray-900 mb-4">{center.name}</h3>
                             <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                                <span className="font-black text-[#8dc63f] text-xs uppercase tracking-widest">Hacer Tour e Instalaciones</span>
      ...
      {view === 'schedule' && (
        <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-12 animate-in slide-in-from-bottom duration-500">
           <div className="mb-12">
              ...
              <p className="text-gray-500 text-lg font-medium">Reserva tu plaza en cualquiera de nuestras clases dirigidas.</p>
           </div>
           ...
           {/* Mobile schedule layout would go here for completeness, but maintaining desktop layout focus */}
        </main>
      )}
